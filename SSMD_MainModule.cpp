...
**************************************************************************************
					...
					************************************
*/
int vPlanetSelect1 = 3;
int vPlanetSelect2 = 4;
int vPlanetSelect3 = 3;  // Assumes only (1) gravity-assist for this code version... 

double vDeltaVmax = 6.00;
//******************************************************************
int vFlybyNo = 2;
//******************************************************************

int vYearInput = 2015;			int vMonthInput = 1;		int vDayInput = 1;
int vYearInputLast = 2025;		int vMonthInputLast = 12;	int vDayInputLast = 31;

double vArc1Min = 90;			double vArc1Max = 365 * 3;

double vArc2Min = 90;			double vArc2Max = 365 * 3;

double vArc3Min = 90;			double vArc3Max = 365 * 3;
...
int main()
{
	...
	double vDeltaVinfFlyby1, vDeltaVinfFlyby2;
	...
	//======================================================================================================================================================================
	// Main Body ===========================================================================================================================================================
	for (vDepartCount = 0; vDepartCount < vDepartMJDspan; vDepartCount++)
	{
		...
		cout << vDepartCount - vDepartCount / 3 << endl;


		//for (jj = vArc1Min; jj <= vArc1Max; jj++)
		for (jj = vArc1Min; jj <= vArc1Max; jj = jj + 1)
		{
			vTOF1 = jj;   vTOF1secs = vTOF1 * 24 * 60 * 60;		vArrive1MJD = vDepartMJD + jj;
			...
			//	if (vDeltaV <= vDeltaVmax)
			//	{
			stPorkChopData1[vArc1noOut].vJDdepart = vDepartMJD;
			stPorkChopData1[vArc1noOut].vJDarrive = vArrive1MJD;
 ...
 ***********************************************************************************************************************************
			...
			if (vec_magntd(aSCarc1DepInfVelVector) <= 8.00 && vFlybyNo > 0)
			{
				//for (kk = vArc2Min; kk <= vArc2Max; kk++)
				for (kk = vArc2Min; kk <= vArc2Max; kk = kk + 2)
				{
					...
					stPlanetPosVel3.aVelVector[1] = stMarsPosVelForJD[vDepartCount + jj + kk].aVelVector[1];
					...
					if (vDeltaVinfFlyby1 <= 0.01 && vSCTurnCompute <= vMaxSCTurn)
					{
						...
						stFlyby1Data[vFlyby1Count].vVinfVelAvg = vFlybyVinfAvgMag;
			...
			jjCount = jjCount + 1; // need to put somewhere else.. 
			vArc1no = vArc1no + 1;
		}
	...
	cout << vDepartCount * (vArc1Max - vArc1Min) << endl;
	cout << "Total script execution speed for this run is:" << endl;
	cout << double(clock() - startTime) / 1000 << endl;

	ofstream PorkChopFile;
	PorkChopFile.open("PorkChop.txt");
	PorkChopFile << "Depart Date (Greg)" << setw(30) << "Arrive-1 Date (Greg)" << setw(30) << "Depart Date (JD)" << setw(30) << " Arrive-1 Date (JD)" << setw(30) << " C3 Magntd (km^2/sec^2)" << setw(30) << "DeltaV" << endl;
	PorkChopFile << setprecision(12);

	for (kk = 0; kk < vArc1noOut; kk++)
	{
		sGregDateOutputDep = JulianDay2Greg(stPorkChopData1[kk].vJDdepart);
		sGregDateOutputArr1 = JulianDay2Greg(stPorkChopData1[kk].vJDarrive);

		PorkChopFile << sGregDateOutputDep.c_str() << setw(30) << sGregDateOutputArr1.c_str() << setw(30) << stPorkChopData1[kk].vJDdepart << setw(30) << stPorkChopData1[kk].vJDarrive << setw(30) << stPorkChopData1[kk].vC3 << setw(30) << stPorkChopData1[kk].vDeltaV << setw(30) << kk << endl;
	}
   ...
   //******************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************************
	ofstream myfile;
	myfile.open("VenusPorkChopPlotRTO.txt");
	myfile << "Depart Date (Greg)" << setw(30) << "Arrive-1 Date (Greg)" << setw(30) << "Arrive-1 Date (Greg)" << setw(30) << "Depart Date (JD)" << setw(30) << " Arrive-1 Date (JD)" << setw(30) << " Arrive-2 Date (JD)" << setw(30) << " C3 Magntd (km^2/sec^2)" << setw(30) << "Total TOF" << setw(30) << " Turn Angle (rad)" << setw(30) << "Vinf Vel Vector Avg Magntd" << setw(30) << "Flyby Altitude" << endl;
	...
	for (kk = 0; kk < vFlyby1Count; kk++)
	{
		...
		myfile << sGregDateOutputDep.c_str() << setw(30) << sGregDateOutputArr1.c_str() << setw(30) << sGregDateOutputArr2.c_str() << setw(30) << stFlyby1Data[kk].vDepartJD << setw(30) << stFlyby1Data[kk].vArrive1JD << setw(30) << stFlyby1Data[kk].vArrive2JD << setw(30) << stFlyby1Data[kk].vC3 << setw(30) << stFlyby1Data[kk].vTotalTOF << setw(30) << stFlyby1Data[kk].vTurnAngleRad << setw(30) << stFlyby1Data[kk].vVinfVelAvg << setw(30) << stFlyby1Data[kk].vFlybyAlt << kk << endl;
	}
	myfile.close();
